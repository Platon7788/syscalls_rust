...
fn main() {
    ...
    unsafe {
        ...
        if NT_SUCCESS(status) && !base_addr.is_null() {
            ...
            core::ptr::copy_nonoverlapping(test_data.as_ptr(), base_addr as *mut u8, 4);
            println!("    OK: Written 0xDEADBEEF\n");

            // Test 4: Read from memory
            println!("[4] Testing memory read...");
            let mut read_data: [u8; 4] = [0; 4];
            core::ptr::copy_nonoverlapping(base_addr as *const u8, read_data.as_mut_ptr(), 4);
            println!(
                "    Read: 0x{:02X}{:02X}{:02X}{:02X}",
    ...
    unsafe {
        ...
        println!(
            "    Status: 0x{:08X} (expected STATUS_INFO_LENGTH_MISMATCH)",
            status as u32
        );
