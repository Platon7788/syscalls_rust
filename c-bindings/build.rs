...
fn main() {
    ...
    let content = fs::read_to_string(syscalls_lib)
        .expect("Failed to read syscalls/lib.rs");
    ...
    let mut file = fs::File::create(output_header)
        .expect("Failed to create header file");
    file.write_all(header.as_bytes())
        .expect("Failed to write header");
    
    let mut wrapper_file = fs::File::create(&output_wrappers)
        .expect("Failed to create wrapper file");
    wrapper_file.write_all(wrappers.as_bytes())
        .expect("Failed to write wrappers");
    
    println!("cargo:warning=Generated {} with {} functions", output_header, functions.len());
}
...
fn extract_functions(content: &str) -> Vec<Function> {
    ...
    let fn_re = regex::Regex::new(
        r#"pub unsafe (?:extern "C" )?fn (\w+)\s*\(([^)]*)\)\s*->\s*(\w+)"#
    ).unwrap();
...
fn generate_base_types(h: &mut String) {
    ...
    for (sw3_name, original_name, c_type) in &sw3_base_types {
        ...
        h.push_str(&format!("#if !SYSCALLS_WINDOWS_SDK_DETECTED\n"));
...
fn generate_opaque_types(h: &mut String) {
    ...
    let sw3_opaque_types = [
        ...
        ("SW3_PFILE_USER_QUOTA_INFORMATION", "PFILE_USER_QUOTA_INFORMATION"),
        ("SW3_PFILE_QUOTA_LIST_INFORMATION", "PFILE_QUOTA_LIST_INFORMATION"), 
        ("SW3_PFILE_IO_COMPLETION_INFORMATION", "PFILE_IO_COMPLETION_INFORMATION"),
...
fn generate_structs(h: &mut String, structs: &[(String, Vec<(String, String)>)]) {
    ...
    for (name, fields) in structs {
        ...
        if fields.is_empty() || fields.iter().all(|(field_name, _)| field_name == "PartitionId" || field_name.starts_with("[")) {
            ...
            match name.as_str() {
                ...
                "WNF_TYPE_ID" => {
                    h.push_str("    SW3_UCHAR TypeId[16];  /* Placeholder to avoid empty struct */\n");
                }
                _ => {
                    h.push_str("    SW3_ULONG Reserved;  /* Placeholder to avoid empty struct */\n");
                }
            }
        } else {
            for (field_name, field_type) in fields {
                ...
                if field_type.starts_with("[") && field_type.contains(";") && field_type.ends_with("]") {
    ...
    h.push_str("/* These structures are needed by syscalls but may not be in all SDK versions */\n\n");
...
fn generate_all_constants(h: &mut String, _constants: &[(String, String, String)]) {
    ...
    let sw3_core_status = [
        ...
        ("SW3_STATUS_UNSUCCESSFUL", "STATUS_UNSUCCESSFUL", "0xC0000001L"),
        ("SW3_STATUS_ACCESS_DENIED", "STATUS_ACCESS_DENIED", "0xC0000022L"),
        ("SW3_STATUS_INVALID_HANDLE", "STATUS_INVALID_HANDLE", "0xC0000008L"),
        ("SW3_STATUS_INVALID_PARAMETER", "STATUS_INVALID_PARAMETER", "0xC000000DL"),
        ("SW3_STATUS_NO_MEMORY", "STATUS_NO_MEMORY", "0xC0000017L"),
        ("SW3_STATUS_BUFFER_TOO_SMALL", "STATUS_BUFFER_TOO_SMALL", "0xC0000023L"),
        ("SW3_STATUS_OBJECT_NAME_NOT_FOUND", "STATUS_OBJECT_NAME_NOT_FOUND", "0xC0000034L"),
        ("SW3_STATUS_OBJECT_PATH_NOT_FOUND", "STATUS_OBJECT_PATH_NOT_FOUND", "0xC000003AL"),
        ("SW3_STATUS_SHARING_VIOLATION", "STATUS_SHARING_VIOLATION", "0xC0000043L"),
        ("SW3_STATUS_INSUFFICIENT_RESOURCES", "STATUS_INSUFFICIENT_RESOURCES", "0xC000009AL"),
        ("SW3_STATUS_NOT_SUPPORTED", "STATUS_NOT_SUPPORTED", "0xC00000BBL"),
        ("SW3_STATUS_INVALID_PARAMETER_1", "STATUS_INVALID_PARAMETER_1", "0xC00000EFL"),
        ("SW3_STATUS_INVALID_PARAMETER_2", "STATUS_INVALID_PARAMETER_2", "0xC00000F0L"),
        ("SW3_STATUS_INVALID_PARAMETER_3", "STATUS_INVALID_PARAMETER_3", "0xC00000F1L"),
        ("SW3_STATUS_INVALID_PARAMETER_4", "STATUS_INVALID_PARAMETER_4", "0xC00000F2L"),
        ("SW3_STATUS_INVALID_PARAMETER_5", "STATUS_INVALID_PARAMETER_5", "0xC00000F3L"),
        ("SW3_STATUS_INVALID_PARAMETER_6", "STATUS_INVALID_PARAMETER_6", "0xC00000F4L"),
        ("SW3_STATUS_PROCEDURE_NOT_FOUND", "STATUS_PROCEDURE_NOT_FOUND", "0xC000007AL"),
        ("SW3_STATUS_INVALID_IMAGE_FORMAT", "STATUS_INVALID_IMAGE_FORMAT", "0xC000007BL"),
        ("SW3_STATUS_NO_TOKEN", "STATUS_NO_TOKEN", "0xC000007CL"),
        ("SW3_STATUS_PRIVILEGE_NOT_HELD", "STATUS_PRIVILEGE_NOT_HELD", "0xC0000061L"),
        ("SW3_STATUS_ENTRYPOINT_NOT_FOUND", "STATUS_ENTRYPOINT_NOT_FOUND", "0xC0000139L"),
        ("SW3_STATUS_DLL_NOT_FOUND", "STATUS_DLL_NOT_FOUND", "0xC0000135L"),
        ("SW3_STATUS_WAIT_0", "STATUS_WAIT_0", "0x00000000L"),
        ("SW3_STATUS_ABANDONED_WAIT_0", "STATUS_ABANDONED_WAIT_0", "0x00000080L"),
        ...
        ("SW3_STATUS_BUFFER_OVERFLOW", "STATUS_BUFFER_OVERFLOW", "0x80000005L"),
        ("SW3_STATUS_NO_MORE_FILES", "STATUS_NO_MORE_FILES", "0x80000006L"),
        ("SW3_STATUS_NO_MORE_ENTRIES", "STATUS_NO_MORE_ENTRIES", "0x8000001AL"),
    ];
    ...
    let sw3_memory_constants = [
        ...
        ("SW3_PAGE_EXECUTE_READWRITE", "PAGE_EXECUTE_READWRITE", "0x40"),
        ("SW3_PAGE_EXECUTE_WRITECOPY", "PAGE_EXECUTE_WRITECOPY", "0x80"),
    ...
    let sw3_access_constants = [
        ...
        ("SW3_STANDARD_RIGHTS_REQUIRED", "STANDARD_RIGHTS_REQUIRED", "0x000F0000"),
        ("SW3_STANDARD_RIGHTS_ALL", "STANDARD_RIGHTS_ALL", "0x001F0000"),
        ("SW3_SPECIFIC_RIGHTS_ALL", "SPECIFIC_RIGHTS_ALL", "0x0000FFFF"),
        ("SW3_PROCESS_TERMINATE", "PROCESS_TERMINATE", "0x0001"),
        ("SW3_PROCESS_CREATE_THREAD", "PROCESS_CREATE_THREAD", "0x0002"),
        ...
        ("SW3_PROCESS_CREATE_PROCESS", "PROCESS_CREATE_PROCESS", "0x0080"),
        ("SW3_PROCESS_SET_QUOTA", "PROCESS_SET_QUOTA", "0x0100"),
        ("SW3_PROCESS_SET_INFORMATION", "PROCESS_SET_INFORMATION", "0x0200"),
        ("SW3_PROCESS_QUERY_INFORMATION", "PROCESS_QUERY_INFORMATION", "0x0400"),
        ("SW3_PROCESS_SUSPEND_RESUME", "PROCESS_SUSPEND_RESUME", "0x0800"),
        ("SW3_PROCESS_QUERY_LIMITED_INFORMATION", "PROCESS_QUERY_LIMITED_INFORMATION", "0x1000"),
        ("SW3_PROCESS_ALL_ACCESS", "PROCESS_ALL_ACCESS", "0x1FFFFF"),
        ("SW3_THREAD_TERMINATE", "THREAD_TERMINATE", "0x0001"),
        ("SW3_THREAD_SUSPEND_RESUME", "THREAD_SUSPEND_RESUME", "0x0002"),
        ("SW3_THREAD_GET_CONTEXT", "THREAD_GET_CONTEXT", "0x0008"),
        ("SW3_THREAD_SET_CONTEXT", "THREAD_SET_CONTEXT", "0x0010"),
        ("SW3_THREAD_SET_INFORMATION", "THREAD_SET_INFORMATION", "0x0020"),
        ("SW3_THREAD_QUERY_INFORMATION", "THREAD_QUERY_INFORMATION", "0x0040"),
        ("SW3_THREAD_SET_THREAD_TOKEN", "THREAD_SET_THREAD_TOKEN", "0x0080"),
        ("SW3_THREAD_IMPERSONATE", "THREAD_IMPERSONATE", "0x0100"),
        ("SW3_THREAD_DIRECT_IMPERSONATION", "THREAD_DIRECT_IMPERSONATION", "0x0200"),
    ...
    let sw3_file_constants = [
        ...
        ("SW3_FILE_WRITE_ATTRIBUTES", "FILE_WRITE_ATTRIBUTES", "0x0100"),
        ...
        ("SW3_FILE_DIRECTORY_FILE", "FILE_DIRECTORY_FILE", "0x00000001"),
        ("SW3_FILE_NON_DIRECTORY_FILE", "FILE_NON_DIRECTORY_FILE", "0x00000040"),
        ("SW3_FILE_SYNCHRONOUS_IO_NONALERT", "FILE_SYNCHRONOUS_IO_NONALERT", "0x00000020"),
        ("SW3_FILE_DELETE_ON_CLOSE", "FILE_DELETE_ON_CLOSE", "0x00001000"),
        ("SW3_REG_SZ", "REG_SZ", "1"),
        ("SW3_REG_OPTION_NON_VOLATILE", "REG_OPTION_NON_VOLATILE", "0x00000000L"),
    ...
    let sw3_registry_constants = [
        ...
        ("SW3_KEY_ENUMERATE_SUB_KEYS", "KEY_ENUMERATE_SUB_KEYS", "0x0008"),
    ...
    let sw3_section_constants = [
        ...
        ("SW3_SECTION_MAP_EXECUTE_EXPLICIT", "SECTION_MAP_EXECUTE_EXPLICIT", "0x0020"),
    ...
    let sw3_token_constants = [
        ...
        ("SW3_TOKEN_ADJUST_PRIVILEGES", "TOKEN_ADJUST_PRIVILEGES", "0x0020"),
        ("SW3_TOKEN_ADJUST_GROUPS", "TOKEN_ADJUST_GROUPS", "0x0040"),
        ("SW3_TOKEN_ADJUST_DEFAULT", "TOKEN_ADJUST_DEFAULT", "0x0080"),
        ("SW3_TOKEN_ADJUST_SESSIONID", "TOKEN_ADJUST_SESSIONID", "0x0100"),
    ...
    let sw3_additional_constants = [
        ...
        ("SW3_SEMAPHORE_MODIFY_STATE", "SEMAPHORE_MODIFY_STATE", "0x0002"),
        ("SW3_TIMER_MODIFY_STATE", "TIMER_MODIFY_STATE", "0x0002"),
        ("SW3_TIMER_QUERY_STATE", "TIMER_QUERY_STATE", "0x0001"),
        
        // Job object constants
        ("SW3_JOB_OBJECT_ASSIGN_PROCESS", "JOB_OBJECT_ASSIGN_PROCESS", "0x0001"),
        ("SW3_JOB_OBJECT_SET_ATTRIBUTES", "JOB_OBJECT_SET_ATTRIBUTES", "0x0002"),
        ("SW3_JOB_OBJECT_QUERY", "JOB_OBJECT_QUERY", "0x0004"),
        ("SW3_JOB_OBJECT_TERMINATE", "JOB_OBJECT_TERMINATE", "0x0008"),
        ("SW3_JOB_OBJECT_SET_SECURITY_ATTRIBUTES", "JOB_OBJECT_SET_SECURITY_ATTRIBUTES", "0x0010"),
        ("SW3_JOB_OBJECT_ALL_ACCESS", "JOB_OBJECT_ALL_ACCESS", "0x1F001F"),
        
        // File attribute constants
        ("SW3_FILE_ATTRIBUTE_READONLY", "FILE_ATTRIBUTE_READONLY", "0x00000001"),
        ("SW3_FILE_ATTRIBUTE_HIDDEN", "FILE_ATTRIBUTE_HIDDEN", "0x00000002"),
        ("SW3_FILE_ATTRIBUTE_SYSTEM", "FILE_ATTRIBUTE_SYSTEM", "0x00000004"),
        ("SW3_FILE_ATTRIBUTE_DIRECTORY", "FILE_ATTRIBUTE_DIRECTORY", "0x00000010"),
        ("SW3_FILE_ATTRIBUTE_ARCHIVE", "FILE_ATTRIBUTE_ARCHIVE", "0x00000020"),
        ("SW3_FILE_ATTRIBUTE_DEVICE", "FILE_ATTRIBUTE_DEVICE", "0x00000040"),
        ("SW3_FILE_ATTRIBUTE_NORMAL", "FILE_ATTRIBUTE_NORMAL", "0x00000080"),
        ("SW3_FILE_ATTRIBUTE_TEMPORARY", "FILE_ATTRIBUTE_TEMPORARY", "0x00000100"),
        ("SW3_FILE_ATTRIBUTE_SPARSE_FILE", "FILE_ATTRIBUTE_SPARSE_FILE", "0x00000200"),
        ("SW3_FILE_ATTRIBUTE_REPARSE_POINT", "FILE_ATTRIBUTE_REPARSE_POINT", "0x00000400"),
        ("SW3_FILE_ATTRIBUTE_COMPRESSED", "FILE_ATTRIBUTE_COMPRESSED", "0x00000800"),
        ("SW3_FILE_ATTRIBUTE_OFFLINE", "FILE_ATTRIBUTE_OFFLINE", "0x00001000"),
        ("SW3_FILE_ATTRIBUTE_NOT_CONTENT_INDEXED", "FILE_ATTRIBUTE_NOT_CONTENT_INDEXED", "0x00002000"),
        ("SW3_FILE_ATTRIBUTE_ENCRYPTED", "FILE_ATTRIBUTE_ENCRYPTED", "0x00004000"),
        ("SW3_FILE_ATTRIBUTE_INTEGRITY_STREAM", "FILE_ATTRIBUTE_INTEGRITY_STREAM", "0x00008000"),
        ("SW3_FILE_ATTRIBUTE_VIRTUAL", "FILE_ATTRIBUTE_VIRTUAL", "0x00010000"),
        ("SW3_FILE_ATTRIBUTE_NO_SCRUB_DATA", "FILE_ATTRIBUTE_NO_SCRUB_DATA", "0x00020000"),
        ("SW3_FILE_ATTRIBUTE_EA", "FILE_ATTRIBUTE_EA", "0x00040000"),
        ("SW3_FILE_ATTRIBUTE_PINNED", "FILE_ATTRIBUTE_PINNED", "0x00080000"),
        ("SW3_FILE_ATTRIBUTE_UNPINNED", "FILE_ATTRIBUTE_UNPINNED", "0x00100000"),
        ("SW3_FILE_ATTRIBUTE_RECALL_ON_OPEN", "FILE_ATTRIBUTE_RECALL_ON_OPEN", "0x00040000"),
        ("SW3_FILE_ATTRIBUTE_RECALL_ON_DATA_ACCESS", "FILE_ATTRIBUTE_RECALL_ON_DATA_ACCESS", "0x00400000"),
        
        // Directory access constants
        ("SW3_FILE_ADD_FILE", "FILE_ADD_FILE", "0x0002"),
        ("SW3_FILE_ADD_SUBDIRECTORY", "FILE_ADD_SUBDIRECTORY", "0x0004"),
        ("SW3_FILE_CREATE_PIPE_INSTANCE", "FILE_CREATE_PIPE_INSTANCE", "0x0004"),
        ...
        ("SW3_PIPE_ACCESS_INBOUND", "PIPE_ACCESS_INBOUND", "0x00000001"),
        ("SW3_PIPE_ACCESS_OUTBOUND", "PIPE_ACCESS_OUTBOUND", "0x00000002"),
        ...
        ("SW3_REG_DWORD_LITTLE_ENDIAN", "REG_DWORD_LITTLE_ENDIAN", "4"),
        ...
        ("SW3_REG_QWORD_LITTLE_ENDIAN", "REG_QWORD_LITTLE_ENDIAN", "11"),
        
        // Security constants
        ("SW3_SECURITY_ANONYMOUS", "SECURITY_ANONYMOUS", "0"),
        ("SW3_SECURITY_IDENTIFICATION", "SECURITY_IDENTIFICATION", "1"),
        ("SW3_SECURITY_IMPERSONATION", "SECURITY_IMPERSONATION", "2"),
        ("SW3_SECURITY_DELEGATION", "SECURITY_DELEGATION", "3"),
        
        // Additional access rights
        ("SW3_ACCESS_SYSTEM_SECURITY", "ACCESS_SYSTEM_SECURITY", "0x01000000"),
        ("SW3_MAXIMUM_ALLOWED", "MAXIMUM_ALLOWED", "0x02000000"),
        ...
        ("SW3_ERROR_TOO_MANY_OPEN_FILES", "ERROR_TOO_MANY_OPEN_FILES", "4"),
        ("SW3_ERROR_ACCESS_DENIED", "ERROR_ACCESS_DENIED", "5"),
        ("SW3_ERROR_INVALID_HANDLE", "ERROR_INVALID_HANDLE", "6"),
        ("SW3_ERROR_NOT_ENOUGH_MEMORY", "ERROR_NOT_ENOUGH_MEMORY", "8"),
        ("SW3_ERROR_INVALID_DATA", "ERROR_INVALID_DATA", "13"),
        ("SW3_ERROR_INVALID_PARAMETER", "ERROR_INVALID_PARAMETER", "87"),
        ("SW3_ERROR_INSUFFICIENT_BUFFER", "ERROR_INSUFFICIENT_BUFFER", "122"),
    ...
    let sw3_privilege_constants = [
        ("SW3_SE_ASSIGNPRIMARYTOKEN_NAME", "SE_ASSIGNPRIMARYTOKEN_NAME", "\"SeAssignPrimaryTokenPrivilege\""),
        ("SW3_SE_AUDIT_NAME", "SE_AUDIT_NAME", "\"SeAuditPrivilege\""),
        ("SW3_SE_BACKUP_NAME", "SE_BACKUP_NAME", "\"SeBackupPrivilege\""),
        ("SW3_SE_CHANGE_NOTIFY_NAME", "SE_CHANGE_NOTIFY_NAME", "\"SeChangeNotifyPrivilege\""),
        ("SW3_SE_CREATE_GLOBAL_NAME", "SE_CREATE_GLOBAL_NAME", "\"SeCreateGlobalPrivilege\""),
        ("SW3_SE_CREATE_PAGEFILE_NAME", "SE_CREATE_PAGEFILE_NAME", "\"SeCreatePagefilePrivilege\""),
        ("SW3_SE_CREATE_PERMANENT_NAME", "SE_CREATE_PERMANENT_NAME", "\"SeCreatePermanentPrivilege\""),
        ("SW3_SE_CREATE_SYMBOLIC_LINK_NAME", "SE_CREATE_SYMBOLIC_LINK_NAME", "\"SeCreateSymbolicLinkPrivilege\""),
        ("SW3_SE_CREATE_TOKEN_NAME", "SE_CREATE_TOKEN_NAME", "\"SeCreateTokenPrivilege\""),
        ("SW3_SE_DEBUG_NAME", "SE_DEBUG_NAME", "\"SeDebugPrivilege\""),
        ("SW3_SE_DELEGATE_SESSION_USER_IMPERSONATE_NAME", "SE_DELEGATE_SESSION_USER_IMPERSONATE_NAME", "\"SeDelegateSessionUserImpersonatePrivilege\""),
        ("SW3_SE_ENABLE_DELEGATION_NAME", "SE_ENABLE_DELEGATION_NAME", "\"SeEnableDelegationPrivilege\""),
        ("SW3_SE_IMPERSONATE_NAME", "SE_IMPERSONATE_NAME", "\"SeImpersonatePrivilege\""),
        ("SW3_SE_INC_BASE_PRIORITY_NAME", "SE_INC_BASE_PRIORITY_NAME", "\"SeIncreaseBasePriorityPrivilege\""),
        ("SW3_SE_INCREASE_QUOTA_NAME", "SE_INCREASE_QUOTA_NAME", "\"SeIncreaseQuotaPrivilege\""),
        ("SW3_SE_INC_WORKING_SET_NAME", "SE_INC_WORKING_SET_NAME", "\"SeIncreaseWorkingSetPrivilege\""),
        ("SW3_SE_LOAD_DRIVER_NAME", "SE_LOAD_DRIVER_NAME", "\"SeLoadDriverPrivilege\""),
        ("SW3_SE_LOCK_MEMORY_NAME", "SE_LOCK_MEMORY_NAME", "\"SeLockMemoryPrivilege\""),
        ("SW3_SE_MACHINE_ACCOUNT_NAME", "SE_MACHINE_ACCOUNT_NAME", "\"SeMachineAccountPrivilege\""),
        ("SW3_SE_MANAGE_VOLUME_NAME", "SE_MANAGE_VOLUME_NAME", "\"SeManageVolumePrivilege\""),
        ("SW3_SE_PROF_SINGLE_PROCESS_NAME", "SE_PROF_SINGLE_PROCESS_NAME", "\"SeProfileSingleProcessPrivilege\""),
        ("SW3_SE_RELABEL_NAME", "SE_RELABEL_NAME", "\"SeRelabelPrivilege\""),
        ("SW3_SE_REMOTE_SHUTDOWN_NAME", "SE_REMOTE_SHUTDOWN_NAME", "\"SeRemoteShutdownPrivilege\""),
        ("SW3_SE_RESTORE_NAME", "SE_RESTORE_NAME", "\"SeRestorePrivilege\""),
        ("SW3_SE_SECURITY_NAME", "SE_SECURITY_NAME", "\"SeSecurityPrivilege\""),
        ("SW3_SE_SHUTDOWN_NAME", "SE_SHUTDOWN_NAME", "\"SeShutdownPrivilege\""),
        ("SW3_SE_SYNC_AGENT_NAME", "SE_SYNC_AGENT_NAME", "\"SeSyncAgentPrivilege\""),
        ("SW3_SE_SYSTEM_ENVIRONMENT_NAME", "SE_SYSTEM_ENVIRONMENT_NAME", "\"SeSystemEnvironmentPrivilege\""),
        ("SW3_SE_SYSTEM_PROFILE_NAME", "SE_SYSTEM_PROFILE_NAME", "\"SeSystemProfilePrivilege\""),
        ("SW3_SE_SYSTEMTIME_NAME", "SE_SYSTEMTIME_NAME", "\"SeSystemtimePrivilege\""),
        ("SW3_SE_TAKE_OWNERSHIP_NAME", "SE_TAKE_OWNERSHIP_NAME", "\"SeTakeOwnershipPrivilege\""),
        ("SW3_SE_TCB_NAME", "SE_TCB_NAME", "\"SeTcbPrivilege\""),
        ("SW3_SE_TIME_ZONE_NAME", "SE_TIME_ZONE_NAME", "\"SeTimeZonePrivilege\""),
        ("SW3_SE_TRUSTED_CREDMAN_ACCESS_NAME", "SE_TRUSTED_CREDMAN_ACCESS_NAME", "\"SeTrustedCredManAccessPrivilege\""),
        ("SW3_SE_UNDOCK_NAME", "SE_UNDOCK_NAME", "\"SeUndockPrivilege\""),
        ("SW3_SE_UNSOLICITED_INPUT_NAME", "SE_UNSOLICITED_INPUT_NAME", "\"SeUnsolicitedInputPrivilege\""),
    ];
    ...
    let sw3_system_info_constants = [
        ...
        ("SW3_SystemProcessorInformation", "SystemProcessorInformation", "1"),
        ("SW3_SystemPerformanceInformation", "SystemPerformanceInformation", "2"),
        ("SW3_SystemTimeOfDayInformation", "SystemTimeOfDayInformation", "3"),
        ("SW3_SystemPathInformation", "SystemPathInformation", "4"),
        ("SW3_SystemProcessInformation", "SystemProcessInformation", "5"),
        ("SW3_SystemCallCountInformation", "SystemCallCountInformation", "6"),
        ("SW3_SystemDeviceInformation", "SystemDeviceInformation", "7"),
        ("SW3_SystemProcessorPerformanceInformation", "SystemProcessorPerformanceInformation", "8"),
        ("SW3_SystemFlagsInformation", "SystemFlagsInformation", "9"),
        ("SW3_SystemCallTimeInformation", "SystemCallTimeInformation", "10"),
        ("SW3_SystemModuleInformation", "SystemModuleInformation", "11"),
        ("SW3_SystemLocksInformation", "SystemLocksInformation", "12"),
        ("SW3_SystemStackTraceInformation", "SystemStackTraceInformation", "13"),
        ("SW3_SystemPagedPoolInformation", "SystemPagedPoolInformation", "14"),
        ("SW3_SystemNonPagedPoolInformation", "SystemNonPagedPoolInformation", "15"),
        ("SW3_SystemHandleInformation", "SystemHandleInformation", "16"),
        ("SW3_SystemObjectInformation", "SystemObjectInformation", "17"),
        ("SW3_SystemPageFileInformation", "SystemPageFileInformation", "18"),
        ("SW3_SystemVdmInstemulInformation", "SystemVdmInstemulInformation", "19"),
        ("SW3_SystemVdmBopInformation", "SystemVdmBopInformation", "20"),
        ("SW3_SystemFileCacheInformation", "SystemFileCacheInformation", "21"),
        ("SW3_SystemPoolTagInformation", "SystemPoolTagInformation", "22"),
        ("SW3_SystemInterruptInformation", "SystemInterruptInformation", "23"),
        ("SW3_SystemDpcBehaviorInformation", "SystemDpcBehaviorInformation", "24"),
        ("SW3_SystemFullMemoryInformation", "SystemFullMemoryInformation", "25"),
        ("SW3_SystemLoadGdiDriverInformation", "SystemLoadGdiDriverInformation", "26"),
        ("SW3_SystemUnloadGdiDriverInformation", "SystemUnloadGdiDriverInformation", "27"),
        ("SW3_SystemTimeAdjustmentInformation", "SystemTimeAdjustmentInformation", "28"),
        ("SW3_SystemSummaryMemoryInformation", "SystemSummaryMemoryInformation", "29"),
        ("SW3_SystemMirrorMemoryInformation", "SystemMirrorMemoryInformation", "30"),
        ("SW3_SystemPerformanceTraceInformation", "SystemPerformanceTraceInformation", "31"),
        ("SW3_SystemObsolete0", "SystemObsolete0", "32"),
        ("SW3_SystemExceptionInformation", "SystemExceptionInformation", "33"),
        ("SW3_SystemCrashDumpStateInformation", "SystemCrashDumpStateInformation", "34"),
        ("SW3_SystemKernelDebuggerInformation", "SystemKernelDebuggerInformation", "35"),
        ("SW3_SystemContextSwitchInformation", "SystemContextSwitchInformation", "36"),
        ("SW3_SystemRegistryQuotaInformation", "SystemRegistryQuotaInformation", "37"),
        ("SW3_SystemExtendServiceTableInformation", "SystemExtendServiceTableInformation", "38"),
        ("SW3_SystemPrioritySeperation", "SystemPrioritySeperation", "39"),
        ("SW3_SystemVerifierAddDriverInformation", "SystemVerifierAddDriverInformation", "40"),
        ("SW3_SystemVerifierRemoveDriverInformation", "SystemVerifierRemoveDriverInformation", "41"),
        ("SW3_SystemProcessorIdleInformation", "SystemProcessorIdleInformation", "42"),
        ("SW3_SystemLegacyDriverInformation", "SystemLegacyDriverInformation", "43"),
        ("SW3_SystemCurrentTimeZoneInformation", "SystemCurrentTimeZoneInformation", "44"),
        ("SW3_SystemLookasideInformation", "SystemLookasideInformation", "45"),
        ("SW3_SystemTimeSlipNotification", "SystemTimeSlipNotification", "46"),
        ("SW3_SystemSessionCreate", "SystemSessionCreate", "47"),
        ("SW3_SystemSessionDetach", "SystemSessionDetach", "48"),
        ("SW3_SystemSessionInformation", "SystemSessionInformation", "49"),
        ("SW3_SystemRangeStartInformation", "SystemRangeStartInformation", "50"),
        ("SW3_SystemVerifierInformation", "SystemVerifierInformation", "51"),
        ("SW3_SystemVerifierThunkExtend", "SystemVerifierThunkExtend", "52"),
        ("SW3_SystemSessionProcessInformation", "SystemSessionProcessInformation", "53"),
        ("SW3_SystemLoadGdiDriverInSystemSpace", "SystemLoadGdiDriverInSystemSpace", "54"),
        ("SW3_SystemNumaProcessorMap", "SystemNumaProcessorMap", "55"),
        ("SW3_SystemPrefetcherInformation", "SystemPrefetcherInformation", "56"),
        ("SW3_SystemExtendedProcessInformation", "SystemExtendedProcessInformation", "57"),
        ("SW3_SystemRecommendedSharedDataAlignment", "SystemRecommendedSharedDataAlignment", "58"),
        ("SW3_SystemComPlusPackage", "SystemComPlusPackage", "59"),
        ("SW3_SystemNumaAvailableMemory", "SystemNumaAvailableMemory", "60"),
        ("SW3_SystemProcessorPowerInformation", "SystemProcessorPowerInformation", "61"),
        ("SW3_SystemEmulationBasicInformation", "SystemEmulationBasicInformation", "62"),
        ("SW3_SystemEmulationProcessorInformation", "SystemEmulationProcessorInformation", "63"),
        ("SW3_SystemExtendedHandleInformation", "SystemExtendedHandleInformation", "64"),
        ("SW3_SystemLostDelayedWriteInformation", "SystemLostDelayedWriteInformation", "65"),
        ("SW3_SystemBigPoolInformation", "SystemBigPoolInformation", "66"),
        ("SW3_SystemSessionPoolTagInformation", "SystemSessionPoolTagInformation", "67"),
        ("SW3_SystemSessionMappedViewInformation", "SystemSessionMappedViewInformation", "68"),
        ("SW3_SystemHotpatchInformation", "SystemHotpatchInformation", "69"),
        ("SW3_SystemObjectSecurityMode", "SystemObjectSecurityMode", "70"),
        ("SW3_SystemWatchdogTimerHandler", "SystemWatchdogTimerHandler", "71"),
        ("SW3_SystemWatchdogTimerInformation", "SystemWatchdogTimerInformation", "72"),
        ("SW3_SystemLogicalProcessorInformation", "SystemLogicalProcessorInformation", "73"),
        ("SW3_SystemWow64SharedInformationObsolete", "SystemWow64SharedInformationObsolete", "74"),
        ("SW3_SystemRegisterFirmwareTableInformationHandler", "SystemRegisterFirmwareTableInformationHandler", "75"),
        ("SW3_SystemFirmwareTableInformation", "SystemFirmwareTableInformation", "76"),
        ("SW3_SystemModuleInformationEx", "SystemModuleInformationEx", "77"),
        ("SW3_SystemVerifierTriageInformation", "SystemVerifierTriageInformation", "78"),
        ("SW3_SystemSuperfetchInformation", "SystemSuperfetchInformation", "79"),
        ("SW3_SystemMemoryListInformation", "SystemMemoryListInformation", "80"),
        ("SW3_SystemFileCacheInformationEx", "SystemFileCacheInformationEx", "81"),
        ("SW3_SystemThreadPriorityClientIdInformation", "SystemThreadPriorityClientIdInformation", "82"),
        ("SW3_SystemProcessorIdleCycleTimeInformation", "SystemProcessorIdleCycleTimeInformation", "83"),
        ("SW3_SystemVerifierCancellationInformation", "SystemVerifierCancellationInformation", "84"),
        ("SW3_SystemProcessorPowerInformationEx", "SystemProcessorPowerInformationEx", "85"),
        ("SW3_SystemRefTraceInformation", "SystemRefTraceInformation", "86"),
        ("SW3_SystemSpecialPoolInformation", "SystemSpecialPoolInformation", "87"),
        ("SW3_SystemProcessIdInformation", "SystemProcessIdInformation", "88"),
        ("SW3_SystemErrorPortInformation", "SystemErrorPortInformation", "89"),
        ("SW3_SystemBootEnvironmentInformation", "SystemBootEnvironmentInformation", "90"),
        ("SW3_SystemHypervisorInformation", "SystemHypervisorInformation", "91"),
        ("SW3_SystemVerifierInformationEx", "SystemVerifierInformationEx", "92"),
        ("SW3_SystemTimeZoneInformation", "SystemTimeZoneInformation", "93"),
        ("SW3_SystemImageFileExecutionOptionsInformation", "SystemImageFileExecutionOptionsInformation", "94"),
        ("SW3_SystemCoverageInformation", "SystemCoverageInformation", "95"),
        ("SW3_SystemPrefetchPatchInformation", "SystemPrefetchPatchInformation", "96"),
        ("SW3_SystemVerifierFaultsInformation", "SystemVerifierFaultsInformation", "97"),
        ("SW3_SystemSystemPartitionInformation", "SystemSystemPartitionInformation", "98"),
        ("SW3_SystemSystemDiskInformation", "SystemSystemDiskInformation", "99"),
        ("SW3_SystemProcessorPerformanceDistribution", "SystemProcessorPerformanceDistribution", "100"),
        ("SW3_SystemNumaProximityNodeInformation", "SystemNumaProximityNodeInformation", "101"),
        ("SW3_SystemDynamicTimeZoneInformation", "SystemDynamicTimeZoneInformation", "102"),
        ("SW3_SystemCodeIntegrityInformation", "SystemCodeIntegrityInformation", "103"),
        ("SW3_SystemProcessorMicrocodeUpdateInformation", "SystemProcessorMicrocodeUpdateInformation", "104"),
        ("SW3_SystemProcessorBrandString", "SystemProcessorBrandString", "105"),
        ("SW3_SystemVirtualAddressInformation", "SystemVirtualAddressInformation", "106"),
        ("SW3_SystemLogicalProcessorAndGroupInformation", "SystemLogicalProcessorAndGroupInformation", "107"),
        ("SW3_SystemProcessorCycleTimeInformation", "SystemProcessorCycleTimeInformation", "108"),
        ("SW3_SystemStoreInformation", "SystemStoreInformation", "109"),
        ("SW3_SystemRegistryAppendString", "SystemRegistryAppendString", "110"),
        ("SW3_SystemAitSamplingValue", "SystemAitSamplingValue", "111"),
        ("SW3_SystemVhdBootInformation", "SystemVhdBootInformation", "112"),
        ("SW3_SystemCpuQuotaInformation", "SystemCpuQuotaInformation", "113"),
        ("SW3_SystemNativeBasicInformation", "SystemNativeBasicInformation", "114"),
        ("SW3_SystemErrorPortTimeouts", "SystemErrorPortTimeouts", "115"),
        ("SW3_SystemLowPriorityIoInformation", "SystemLowPriorityIoInformation", "116"),
        ("SW3_SystemTpmBootEntropyInformation", "SystemTpmBootEntropyInformation", "117"),
        ("SW3_SystemVerifierCountersInformation", "SystemVerifierCountersInformation", "118"),
        ("SW3_SystemPagedPoolInformationEx", "SystemPagedPoolInformationEx", "119"),
        ("SW3_SystemSystemPtesInformationEx", "SystemSystemPtesInformationEx", "120"),
        ("SW3_SystemNodeDistanceInformation", "SystemNodeDistanceInformation", "121"),
        ("SW3_SystemAcpiAuditInformation", "SystemAcpiAuditInformation", "122"),
        ("SW3_SystemBasicPerformanceInformation", "SystemBasicPerformanceInformation", "123"),
        ("SW3_SystemQueryPerformanceCounterInformation", "SystemQueryPerformanceCounterInformation", "124"),
        ("SW3_SystemSessionBigPoolInformation", "SystemSessionBigPoolInformation", "125"),
        ("SW3_SystemBootGraphicsInformation", "SystemBootGraphicsInformation", "126"),
        ("SW3_SystemScrubPhysicalMemoryInformation", "SystemScrubPhysicalMemoryInformation", "127"),
        ("SW3_SystemBadPageInformation", "SystemBadPageInformation", "128"),
        ("SW3_SystemProcessorProfileControlArea", "SystemProcessorProfileControlArea", "129"),
        ("SW3_SystemCombinePhysicalMemoryInformation", "SystemCombinePhysicalMemoryInformation", "130"),
        ("SW3_SystemEntropyInterruptTimingInformation", "SystemEntropyInterruptTimingInformation", "131"),
        ("SW3_SystemConsoleInformation", "SystemConsoleInformation", "132"),
        ("SW3_SystemPlatformBinaryInformation", "SystemPlatformBinaryInformation", "133"),
        ("SW3_SystemPolicyInformation", "SystemPolicyInformation", "134"),
        ("SW3_SystemHypervisorProcessorCountInformation", "SystemHypervisorProcessorCountInformation", "135"),
        ("SW3_SystemDeviceDataInformation", "SystemDeviceDataInformation", "136"),
        ("SW3_SystemDeviceDataEnumerationInformation", "SystemDeviceDataEnumerationInformation", "137"),
        ("SW3_SystemMemoryTopologyInformation", "SystemMemoryTopologyInformation", "138"),
        ("SW3_SystemMemoryChannelInformation", "SystemMemoryChannelInformation", "139"),
        ("SW3_SystemBootLogoInformation", "SystemBootLogoInformation", "140"),
        ("SW3_SystemProcessorPerformanceInformationEx", "SystemProcessorPerformanceInformationEx", "141"),
        ("SW3_SystemCriticalProcessErrorLogInformation", "SystemCriticalProcessErrorLogInformation", "142"),
        ("SW3_SystemSecureBootPolicyInformation", "SystemSecureBootPolicyInformation", "143"),
        ("SW3_SystemPageFileInformationEx", "SystemPageFileInformationEx", "144"),
        ("SW3_SystemSecureBootInformation", "SystemSecureBootInformation", "145"),
        ("SW3_SystemEntropyInterruptTimingRawInformation", "SystemEntropyInterruptTimingRawInformation", "146"),
        ("SW3_SystemPortableWorkspaceEfiLauncherInformation", "SystemPortableWorkspaceEfiLauncherInformation", "147"),
        ("SW3_SystemFullProcessInformation", "SystemFullProcessInformation", "148"),
        ("SW3_SystemKernelDebuggerInformationEx", "SystemKernelDebuggerInformationEx", "149"),
        ("SW3_SystemBootMetadataInformation", "SystemBootMetadataInformation", "150"),
        ("SW3_SystemSoftRebootInformation", "SystemSoftRebootInformation", "151"),
        ("SW3_SystemElamCertificateInformation", "SystemElamCertificateInformation", "152"),
        ("SW3_SystemOfflineDumpConfigInformation", "SystemOfflineDumpConfigInformation", "153"),
        ("SW3_SystemProcessorFeaturesInformation", "SystemProcessorFeaturesInformation", "154"),
        ("SW3_SystemRegistryReconciliationInformation", "SystemRegistryReconciliationInformation", "155"),
        ("SW3_SystemEdidInformation", "SystemEdidInformation", "156"),
        ("SW3_SystemManufacturingInformation", "SystemManufacturingInformation", "157"),
        ("SW3_SystemEnergyEstimationConfigInformation", "SystemEnergyEstimationConfigInformation", "158"),
        ("SW3_SystemHypervisorDetailInformation", "SystemHypervisorDetailInformation", "159"),
        ("SW3_SystemProcessorCycleStatsInformation", "SystemProcessorCycleStatsInformation", "160"),
        ("SW3_SystemVmGenerationCountInformation", "SystemVmGenerationCountInformation", "161"),
        ("SW3_SystemTrustedPlatformModuleInformation", "SystemTrustedPlatformModuleInformation", "162"),
        ("SW3_SystemKernelDebuggerFlags", "SystemKernelDebuggerFlags", "163"),
        ("SW3_SystemCodeIntegrityPolicyInformation", "SystemCodeIntegrityPolicyInformation", "164"),
        ("SW3_SystemIsolatedUserModeInformation", "SystemIsolatedUserModeInformation", "165"),
        ("SW3_SystemHardwareSecurityTestInterfaceResultsInformation", "SystemHardwareSecurityTestInterfaceResultsInformation", "166"),
        ("SW3_SystemSingleModuleInformation", "SystemSingleModuleInformation", "167"),
        ("SW3_SystemAllowedCpuSetsInformation", "SystemAllowedCpuSetsInformation", "168"),
        ("SW3_SystemVsmProtectionInformation", "SystemVsmProtectionInformation", "169"),
        ("SW3_SystemInterruptCpuSetsInformation", "SystemInterruptCpuSetsInformation", "170"),
        ("SW3_SystemSecureBootPolicyFullInformation", "SystemSecureBootPolicyFullInformation", "171"),
        ("SW3_SystemCodeIntegrityPolicyFullInformation", "SystemCodeIntegrityPolicyFullInformation", "172"),
        ("SW3_SystemAffinitizedInterruptProcessorInformation", "SystemAffinitizedInterruptProcessorInformation", "173"),
        ("SW3_SystemRootSiloInformation", "SystemRootSiloInformation", "174"),
        ("SW3_SystemCpuSetInformation", "SystemCpuSetInformation", "175"),
        ("SW3_SystemCpuSetTagInformation", "SystemCpuSetTagInformation", "176"),
        ("SW3_SystemWin32WerStartCallout", "SystemWin32WerStartCallout", "177"),
        ("SW3_SystemSecureKernelProfileInformation", "SystemSecureKernelProfileInformation", "178"),
        ("SW3_SystemCodeIntegrityPlatformManifestInformation", "SystemCodeIntegrityPlatformManifestInformation", "179"),
        ("SW3_SystemInterruptSteeringInformation", "SystemInterruptSteeringInformation", "180"),
        ("SW3_SystemSupportedProcessorArchitectures", "SystemSupportedProcessorArchitectures", "181"),
        ("SW3_SystemMemoryUsageInformation", "SystemMemoryUsageInformation", "182"),
        ("SW3_SystemCodeIntegrityCertificateInformation", "SystemCodeIntegrityCertificateInformation", "183"),
        ("SW3_SystemPhysicalMemoryInformation", "SystemPhysicalMemoryInformation", "184"),
        ("SW3_SystemControlFlowTransition", "SystemControlFlowTransition", "185"),
        ("SW3_SystemKernelDebuggingAllowed", "SystemKernelDebuggingAllowed", "186"),
        ("SW3_SystemActivityModerationExeState", "SystemActivityModerationExeState", "187"),
        ("SW3_SystemActivityModerationUserSettings", "SystemActivityModerationUserSettings", "188"),
        ("SW3_SystemCodeIntegrityPoliciesFullInformation", "SystemCodeIntegrityPoliciesFullInformation", "189"),
        ("SW3_SystemCodeIntegrityUnlockInformation", "SystemCodeIntegrityUnlockInformation", "190"),
        ("SW3_SystemIntegrityQuotaInformation", "SystemIntegrityQuotaInformation", "191"),
        ("SW3_SystemFlushInformation", "SystemFlushInformation", "192"),
        ("SW3_SystemProcessorIdleMaskInformation", "SystemProcessorIdleMaskInformation", "193"),
        ("SW3_SystemSecureDumpEncryptionInformation", "SystemSecureDumpEncryptionInformation", "194"),
        ("SW3_SystemWriteConstraintInformation", "SystemWriteConstraintInformation", "195"),
        ("SW3_SystemKernelVaShadowInformation", "SystemKernelVaShadowInformation", "196"),
        ("SW3_SystemHypervisorSharedPageInformation", "SystemHypervisorSharedPageInformation", "197"),
        ("SW3_SystemFirmwareBootPerformanceInformation", "SystemFirmwareBootPerformanceInformation", "198"),
        ("SW3_SystemCodeIntegrityVerificationInformation", "SystemCodeIntegrityVerificationInformation", "199"),
        ("SW3_SystemFirmwarePartitionInformation", "SystemFirmwarePartitionInformation", "200"),
        ("SW3_SystemSpeculationControlInformation", "SystemSpeculationControlInformation", "201"),
        ("SW3_SystemDmaGuardPolicyInformation", "SystemDmaGuardPolicyInformation", "202"),
        ("SW3_SystemEnclaveLaunchControlInformation", "SystemEnclaveLaunchControlInformation", "203"),
        ("SW3_SystemWorkloadAllowedCpuSetsInformation", "SystemWorkloadAllowedCpuSetsInformation", "204"),
        ("SW3_SystemCodeIntegrityUnlockModeInformation", "SystemCodeIntegrityUnlockModeInformation", "205"),
        ("SW3_SystemLeapSecondInformation", "SystemLeapSecondInformation", "206"),
        ("SW3_SystemFlags2Information", "SystemFlags2Information", "207"),
        ("SW3_SystemSecurityModelInformation", "SystemSecurityModelInformation", "208"),
        ("SW3_SystemCodeIntegritySyntheticCacheInformation", "SystemCodeIntegritySyntheticCacheInformation", "209"),
    ...
    let sw3_info_constants = [
        ...
        ("SW3_KeyValueFullInformation", "KeyValueFullInformation", "1"),
        ("SW3_ProcessBasicInformation", "ProcessBasicInformation", "0"),
    ...
    let sw3_obj_constants = [
        ...
        ("SW3_OBJ_CASE_INSENSITIVE", "OBJ_CASE_INSENSITIVE", "0x00000040"),
        ...
        ("SW3_OBJ_FORCE_ACCESS_CHECK", "OBJ_FORCE_ACCESS_CHECK", "0x00000400"),
        ("SW3_OBJ_VALID_ATTRIBUTES", "OBJ_VALID_ATTRIBUTES", "0x000007F2"),
    ];
...
fn generate_functions(h: &mut String, functions: &[Function]) {
    ...
    h.push_str(&format!("/* Total functions: {} (all with SW3_ prefix for compatibility) */\n\n", functions.len()));
    ...
    h.push_str("/* Aliases for backward compatibility - map original names to SW3 prefixed names */\n\n");
    ...
    h.push_str("/* Note: All functions are always available regardless of Windows SDK detection */\n");
...
fn generate_c_wrappers(functions: &[Function]) -> String {
    ...
    for func in functions {
        ...
        w.push_str("#[no_mangle]\n");
        w.push_str(&format!("pub unsafe extern \"C\" fn {}({}) -> {} {{\n", 
    ...
    w.push_str(&format!("// Generated {} C wrappers for NT syscall functions\n", syscall_count));
    ...
    w.push_str(&format!("// Total exported functions: {}\n", syscall_count + 6));
