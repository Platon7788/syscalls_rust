# SysWhispers3 Rust Syscalls

Direct Windows NT syscall library, originally generated by
[SysWhispers3](https://github.com/klezVirus/SysWhispers3) and hand-extended
with full WoW64 support, C/C++ bindings, and edition-2024 modernisation.
...
|-----------|-------|-------------|
| **Seed** | `0xB8A54425` | Compile-time seed for name-hash obfuscation |
| **Architecture** | `x86_64 + x86` | Both host-native x64 and WoW64 x86 |
| **Recovery Method** | `JUMPER_RANDOMIZED` | Jump to a random `syscall; ret` slide in ntdll (spoofs return address) |
| **WoW64 Support** | `True` | All 513 stubs have an `#[cfg(target_arch = "x86")]` variant that detects and uses the WoW64 gate at runtime |
| **Total Functions** | `513` | Full SW3-generated NT surface |
...
This crate builds on **stable Rust 1.96+** (edition 2024). No nightly
...
The following sibling projects in the Rust_Projects workspace depend on
this crate via path deps. When touching the public API, sanity-check
them before pushing:

| Project | Crate | Path |
|---|---|---|
| IMGUI_NXT | `engine`, `engine-loading` | `IMGUI_NXT/crates/engine*` |
| IMGUI_SPF | `engine` | `IMGUI_SPF/crates/engine` |
| NX_DRV-MOD | `engine` | `NX_DRV-MOD/crates/engine` |
| PE-Protect | `stub` | `PE-Protect/crates/stub` |
| Vex0r/Client | `introspect`, `usermode_debugger` | `Vex0r/Client/crates/*` |
| Auth-Workspace | `client-sdk-windows` | `Auth-Workspace/services/client-sdk-windows` |
| useful-lib | `proc_enum`, `drv_enum` | `useful-lib/*` |

---

*Originally generated by SysWhispers3 Rust Generator, then hand-extended
for WoW64 + edition 2024. See `Docs/CHANGELOG.md` for the history.*
